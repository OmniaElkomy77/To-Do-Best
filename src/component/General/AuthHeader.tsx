...
export default function AuthHeader(props: IHeader) {
    const { title, text, clickable_text,
        back_icon, onPress, navigation, photo, onPress2, activate_account } = props
    return (
        <View style={styles.container}>
            ...
            {activate_account &&
                <View style={styles.text_container}>
                    <AppText text={"Account isn't activated? "} />
                    <TouchableOpacity
                        onPress={onPress2}>
                        <Text style={styles.clickable_text}>{"Active account"}</Text>
                    </TouchableOpacity>
                </View>
            }
        </View>
