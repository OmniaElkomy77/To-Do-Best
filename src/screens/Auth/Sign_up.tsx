...
const SignUp = (props: Props) => {
    ...
    const [selectedAvatar, setSelectedAvatar] = useState<{ avatar: any, name: string, id: string } | null>(null);

    const { navigation } = props;
    const allavatar = async () => {
        try {
            const data = await GetAllAvatars();
            return data;  // Return the resolved avatars array
        } catch (error) {
            console.error("Error fetching avatars:", error);
            // ToastAndroid.show('Error fetching avatars, please try again!', ToastAndroid.SHORT);
            return [];
        }
    }
    const { values, handleChange, handleBlur, errors, handleSubmit, touched } = useFormik({
        ...SignUpSchema,
        onSubmit: async (itemValues: IUserData) => {

            let avatarToSend = selectedAvatar;

            if (!avatarToSend) {
                try {
                    const avatars = await allavatar(); // Fetch all avatars
                    const randomAvatar = avatars[Math.floor(Math.random() * avatars.length)]; // Pick a random one
                    avatarToSend = { avatar: randomAvatar.avatar, name: randomAvatar.name, id: randomAvatar.id };
                } catch (error) {
                    console.error("Error fetching avatars:", error);
                    // Handle the error, maybe show a toast or alert to the user
                    // ToastAndroid.show('Error fetching avatars, please try again!', ToastAndroid.SHORT);
                    return;
                }
            }
